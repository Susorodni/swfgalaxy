function loadBoxes () {
    // BTD5
    document.getElementById("t1").innerHTML = "BTD5";
    document.getElementById("i1").src = "images/btd5.jpg";
    document.getElementById("b1").onclick = function () { btd5(); };
    // Chibi Knight
    document.getElementById("n3").onclick = function () { chibiknight(); };
    // Duck Life 4
    document.getElementById("t2").innerHTML = "Duck Life 4";
    document.getElementById("i2").src = "images/ducklife-4.jpeg";
    document.getElementById("b2").onclick = function () { ducklife4(); };
    // Flappy Bird
    document.getElementById("t3").innerHTML = "Flappy Bird";
    document.getElementById("i3").src = "images/flappybird.jpg";
    document.getElementById("b3").onclick = function () { flappybird(); };
    // Geometry Dash
    document.getElementById("t4").innerHTML = "Geometry Dash";
    document.getElementById("i4").src = "images/geometrydash.jpg";
    document.getElementById("b4").onclick = function () { geodash(); };
    // Give Up 2
    document.getElementById("t5").innerHTML = "Give Up 2";
    document.getElementById("i5").src = "images/giveup2.jpeg";
    document.getElementById("b5").onclick = function () { giveup2(); };
    // Happy Wheels
    document.getElementById("t6").innerHTML = "Happy Wheels";
    document.getElementById("i6").src = "images/happywheels.jpeg";
    document.getElementById("b6").onclick = function () { happywheels(); };
    // Impossible Quiz
    document.getElementById("n1").onclick = function () { impossiblequiz(); };
    // Jacksmith
    document.getElementById("t7").innerHTML = "Jacksmith";
    document.getElementById("i7").src = "images/jacksmith.jpeg";
    document.getElementById("b7").onclick = function () { jacksmith(); };
    // Learn to Fly 3
    document.getElementById("t8").innerHTML = "Learn to Fly 3";
    document.getElementById("i8").src = "images/learntofly3.jpeg";
    document.getElementById("b8").onclick = function () { learntofly3(); };
    // Papa's Bakeria
    document.getElementById("t9").innerHTML = "Papa's Bakeria";
    document.getElementById("i9").src = "images/papasbakeria.jpeg";
    document.getElementById("b9").onclick = function () { papasbakeria(); };
    // Papa's Donuteria
    document.getElementById("t10").innerHTML = "Papa's Donuteria";
    document.getElementById("i10").src = "images/papasdonuteria.jpeg";
    document.getElementById("b10").onclick = function () { papasdonuteria(); };
    // Pokemon Tower Defense
    document.getElementById("t11").innerHTML = "Pokemon Tower Defense";
    document.getElementById("i11").src = "images/pokemontowerdefense.jpeg";
    document.getElementById("b11").onclick = function () { pokemontowerdefense(); };
    // Run 3
    document.getElementById("n2").onclick = function () { run3(); };
    // Strike Force Heroes
    document.getElementById("t12").innerHTML = "Strike Force Heroes";
    document.getElementById("i12").src = "images/strikeforceheroes.jpeg";
    document.getElementById("b12").onclick = function () { strikeforceheroes(); };
    // Strike Force Heroes 2
    document.getElementById("t13").innerHTML = "Strike Force Heroes 2";
    document.getElementById("i13").src = "images/strikeforceheroes2.jpeg";
    document.getElementById("b13").onclick = function () { strikeforceheroes2(); };
    // Sports Heads Basketball Championship
    document.getElementById("t14").innerHTML = "Sports Heads Basketball Championship";
    document.getElementById("i14").src = "images/shbc.jpeg";
    document.getElementById("b14").onclick = function () { sportsheadsbasketball(); };
    // Super Smash Flash
    document.getElementById("t15").innerHTML = "Super Smash Flash";
    document.getElementById("i15").src = "images/supersmashflash.jpeg";
    document.getElementById("b15").onclick = function () { supersmashflash(); };
    // Swords and Sandals 2
    document.getElementById("t16").innerHTML = "Swords and Sandals 2";
    document.getElementById("i16").src = "images/swordsandsandals2.jpg";
    document.getElementById("b16").onclick = function () { swordsandsandals2(); };
    // Tank Trouble
    document.getElementById("t17").innerHTML = "Tank Trouble";
    document.getElementById("i17").src = "images/tanktrouble.jpeg";
    document.getElementById("b17").onclick = function () { tanktrouble(); };
    // Legend of Zelda
    document.getElementById("t18").innerHTML = "Legend of Zelda";
    document.getElementById("i18").src = "images/legendofzelda.jpeg";
    document.getElementById("b18").action = "https://neonk.github.io/zelda/www/index.html";
    // Zombocalypse
    document.getElementById("t19").innerHTML = "Zombocalypse";
    document.getElementById("i19").src = "images/zombo.jpeg";
    document.getElementById("b19").onclick = function () { zombocalypse(); };
}
window.onload = loadBoxes
function btd5() {
        location.href = "btd5.html";
};
function ducklife4() {
        location.href = "ducklife4.html";
};
function flappybird() {
        location.href = "flappybird.html";
};
function geodash() {
        location.href = "geometrydash.html";
};
function giveup2() {
        location.href = "giveup2.html";
};
function happywheels() {
        location.href = "happywheels.html";
};
function jacksmith() {
        location.href = "jacksmith.html";
};
function learntofly3() {
        location.href = "learntofly3.html";
};
function papasbakeria() {
        location.href = "papasbakeria.html";
};
function papasdonuteria() {
        location.href = "papasdonuteria.html";
};
function pokemontowerdefense() {
        location.href = "pokemontowerdefense.html";
};
function strikeforceheroes() {
        location.href = "strikeforceheroes.html";
};
function strikeforceheroes2() {
        location.href = "strikeforceheroes2.html";
};
function sportsheadsbasketball() {
        location.href = "sportsheadsbasketballchampionship.html";
};
function supersmashflash() {
        location.href = "supersmashflash.html";
};
function swordsandsandals2() {
        location.href = "swordsandsandals2.html";
};
function tanktrouble() {
        location.href = "tanktrouble.html";
};
function zombocalypse() {
        location.href = "zombocalypse.html";
};
function impossiblequiz() {
        location.href = "impossiblequiz.html"
};
function run3() {
        location.href = "run3.html"
}
function chibiknight() {
        location.href = "chibiknight.html"
}
